...
(function (global) {
  ...
  // ─── Format compatible avec le dette_data.json historique du projet ──────
  // Convertit le payload structuré en : { data: [{period, dette_pib, dette_montant}, ...], last_update }
  // C'est le format qu'attend le script inline d'index.html (sans rien modifier d'autre).
  async function fetchAsLegacyFormat(options = {}) {
    const payload = await fetchAll(options);
    const detteHist = payload.history.detteAPU || [];
    const pibHist = payload.history.ratioPIB || [];

    // Construction d'un index période → ratio PIB pour jointure
    const pibIndex = {};
    pibHist.forEach(o => { pibIndex[o.period] = o.value; });

    // Tableau aligné chronologiquement (ascendant), au format legacy
    const data = detteHist.map(o => ({
      period: o.period,
      dette_montant: o.value,
      dette_pib: pibIndex[o.period] !== undefined ? pibIndex[o.period] : null,
    })).filter(o => o.dette_pib !== null);

    return {
      data,
      last_update: payload.fetchedAt,
      _source: payload.source,        // 'insee-live' | 'cache' | 'fallback-json'
      _period: payload.period,        // ex. "2025-Q4"
      _full: payload,                 // payload structuré complet (pour usage avancé)
    };
  }

  // ─── Helper UI : actualise automatiquement les badges "T2 2025" → "T4 2025" ──
  // Ne touche que les badges contenant un pattern Tx 20xx, laisse les autres intacts.
  function refreshTrimestreBadges(periodString, selector = '.badge-source') {
    if (!periodString) return 0;
    const formatted = formatPeriod(periodString); // "2025-Q4" → "T4 2025"
    let count = 0;
    document.querySelectorAll(selector).forEach(el => {
      const original = el.textContent;
      const updated = original.replace(/T[1-4]\s*20\d{2}/, formatted);
      if (updated !== original) {
